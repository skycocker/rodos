class FbRelationshipsController < ApplicationController
    ...
    respond_to do |format|
      if @destGroup
        ...
        if @fbrelationship.save
          ...
          format.json { render json: "This group is already connected to group #{@destGroup.name}.", status: :conflict }
        end
      else
  ...
  def validate_group
    unless current_user.groups.find_by_id(params[:rodos_group])
      respond_to do |format|
